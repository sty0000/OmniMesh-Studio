...
2. 页面首次加载时，可能会弹出一个弹窗提示“初始化服务配置”。
3. 点击“跳过，使用默认配置”，然后在右侧抽屉的“系统设置”中，如果 API 地址不对，可以**手动将 API 地址改为 `http://192.168.1.100:8000/v1`** 即可开始聊天！

*(注意：内网访问需要确保服务器的防火墙（如 `ufw` 或 `iptables`）放行了 3000 和 8000 端口。)*
