...
createApp({
  setup() {
    ...
    const onMaxTokensCommit = () => {
      ...
      const normalized =
        Number.isFinite(parsed) && parsed > 0 ? Math.floor(parsed) : fallbackMaxTokens;
    ...
    const estimateCharsPerTick = (pendingLength) => {
      ...
      return Math.min(STREAM_MAX_CHARS_PER_TICK, Math.max(STREAM_MIN_CHARS_PER_TICK, dynamic));
    };
    ...
    const systemPromptTemplates = [
      ...
      },
      {
        ...
        content: '你是Qwen3-Omni-Thinking，现在是<时间>。请在回答时结合当前时间提供最准确的信息。',
      },
      {
        name: '前端开发 (Vue/React)',
        content:
    ...
    const getMultimodalKindConfig = (kind) =>
      config.multimodal?.kinds?.[kind] || { enabled: false, maxBytes: 0, acceptedMimeTypes: [] };

    const getMediaKind = (file) => {
      const mimeType = String(file?.type || '').toLowerCase();
      if (getMultimodalKindConfig('image').acceptedMimeTypes?.includes(mimeType)) return 'image';
      if (getMultimodalKindConfig('audio').acceptedMimeTypes?.includes(mimeType)) return 'audio';
      if (getMultimodalKindConfig('video').acceptedMimeTypes?.includes(mimeType)) return 'video';
      if (getMultimodalKindConfig('file').acceptedMimeTypes?.includes(mimeType)) return 'file';
      return null;
    };

    const isAnyMultimodalEnabled = computed(() =>
      Object.values(config.multimodal?.kinds || {}).some((item) => item?.enabled),
    );

    const acceptedUploadMimeTypes = computed(() =>
      Object.values(config.multimodal?.kinds || {})
        .filter((item) => item?.enabled)
        .flatMap((item) => item.acceptedMimeTypes || [])
        .join(','),
    );

    const canSendMessage = computed(() => {
      const hasReadyMedia = pendingMedia.value.some((item) => item.status === 'ready');
      const hasUploadingMedia = pendingMedia.value.some((item) => item.status === 'uploading');
      const hasErroredMedia = pendingMedia.value.some((item) => item.status === 'error');
      return (
        Boolean(inputPrompt.value.trim() || hasReadyMedia) && !hasUploadingMedia && !hasErroredMedia
      );
    });

    const createPendingUpload = (file, kind) => ({
    ...
    const uploadMediaFiles = async (files) => {
      const uploaded = await api.uploadFrontendFiles({ files, apiKey: config.apiKey });
    ...
    const addMediaFiles = async (files) => {
      ...
      const maxAttachments = Number(config.multimodal?.maxAttachments || 4);
      const readyOrPendingCount = pendingMedia.value.filter(
        (item) => item.status !== 'error',
      ).length;
      if (readyOrPendingCount + files.length > maxAttachments) {
        mediaError.value = `最多只能上传 ${maxAttachments} 个附件`;
        return;
      }
      ...
      for (const file of files) {
        ...
        const kindConfig = kind ? getMultimodalKindConfig(kind) : null;
        if (!kind || !kindConfig?.enabled) {
          mediaError.value = `不支持的文件格式：${file.name || file.type || 'unknown'}`;
          return;
        }
        if (file.size > kindConfig.maxBytes) {
          mediaError.value = `文件 ${file.name} 超过 ${Math.round(kindConfig.maxBytes / 1024 / 1024)}MB 限制`;
          return;
        }
        acceptedFiles.push(file);
        pendingItems.push(createPendingUpload(file, kind));
      }

      pendingMedia.value.push(...pendingItems.map((item) => ({ ...item, status: 'uploading' })));

      try {
        const uploaded = await uploadMediaFiles(acceptedFiles);
        pendingItems.forEach((pending, index) => {
          const target = pendingMedia.value.find((item) => item.id === pending.id);
          if (target) {
            Object.assign(target, uploaded[index] || {}, { id: pending.id });
          }
        });
      } catch (error) {
        mediaError.value = error?.message || '上传失败';
        pendingItems.forEach((pending) => {
          const target = pendingMedia.value.find((item) => item.id === pending.id);
          if (target) {
            target.status = 'error';
            target.error = mediaError.value;
          }
        });
      }
    };
    ...
    const buildPayload = (promptStr, mediaItems = []) => {
      ...
      if (currentSessionSystemPrompt.value && currentSessionSystemPrompt.value.trim() !== '') {
      ...
      if (mediaItems.length > 0) {
        currentContent = createMultimodalContent({ prompt: promptStr, mediaItems });
      } else {
    ...
    const sendMessage = () => {
      ...
      const mediaItems = pendingMedia.value.filter((item) => item.status === 'ready');

      if (!canSendMessage.value) {
        if (!prompt && mediaItems.length === 0) {
          mediaError.value = '请输入提示词或上传至少一个附件';
        }
        return;
      }
